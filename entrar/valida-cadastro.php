...
if (!empty($erros)) {
    ...
    if ($_POST["captcha"] == $_SESSION["palavra"]) {
        ...
        if ($con->query($inserir_usuario) === TRUE) {
            $json_str = '{"status": "ok"}';
            header("Location: entrar.php"); // Redireciona para página de entrar.php com mensagem de sucesso
            exit();
        } else {
            $json_str = '{"status": "erro"}';
            header("Location: entrar.php"); // Redireciona para página de entrar.php com mensagem de erro
            exit();
        }
    } else {
